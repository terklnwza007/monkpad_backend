...
#ลบ transaction ตาม transaction_id 
#ถ้าลบ transaction จะต้องลดยอดใน tags และ month_results ด้วย

#ถ้า tag เป็นรายรับ ให้ลดยอด income ใน month_results
#ถ้า tag เป็นรายจ่าย ให้ลดยอด expense ใน month_results

#ถ้าเดือนนั้น ๆ ใน month_results ไม่มี transaction แล้ว ให้ลบ record นั้นทิ้งไปเลย
@router.delete("/delete/{transaction_id}")
def delete_transaction(transaction_id: int, db: Session = Depends(get_db)):
    # ตรวจสอบว่า transaction มีอยู่จริงไหม
    tr = db.execute(
    ...
    if mr:
        ...
        # ถ้าเดือนนั้น ๆ ไม่มี income และ expense แล้ว ให้ลบ record ทิ้ง
        if new_income == 0 and new_expense == 0:
            db.execute(
                text('DELETE FROM "month_results" WHERE id = :id'),
                {"id": mr.id}
            )
    db.commit()
    return {"message": "Transaction deleted successfully"}
