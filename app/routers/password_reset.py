...
# ---------- CONFIG ----------
OTP_TTL_MINUTES = 10
OTP_MIN_INTERVAL_SECONDS = 60  # ขอใหม่ได้ทุก 60 วินาที
...
def reset_password_with_otp(payload: dict = Body(...), db: Session = Depends(get_db)):
    email = (payload.get("email") or "").strip().lower()
    ...
    try:
        # mark used ของ OTP ล่าสุด
        db.execute(text('UPDATE "password_resets" SET used_at = NOW() WHERE id = :id'),
                   {"id": otp_row._mapping["id"]})
        # อัปเดต user password
        db.execute(text('UPDATE "users" SET password = :p WHERE id = :uid'),
                   {"p": new_hash, "uid": uid})
        # ปิด OTP อื่น ๆ ที่ยังค้าง
        db.execute(text('UPDATE "password_resets" SET used_at = NOW() WHERE user_id = :uid AND used_at IS NULL'),
                   {"uid": uid})
        db.commit()
